...
$('body').on('click', '[data-fancybox-with-slidehow]', function (e) {
	var $this = $(e.target);
	if ($this.is('.slick-slide') || $this.closest('.slick-slide').length > 0) {
		...
		//.index('[data-fancybox-with-slidehow]')
...
$(document).ready(function () {
	...
	$(document).on('click', 'body [data-fancybox-with-slidehow]', function (e) {
		e.preventDefault();
		e.stopPropagation();

		console,log('click');
		var $this = $(e.target);
		var group = $(this).attr('data-fancybox-with-slidehow');
		if (group !== undefined && group !== '') {
			group = '="' + group + '"';
		}
		var items = $('[data-fancybox-with-slidehow' + group + ']');
		...
		$.fancybox.open(itemsHrefs, {
			...
			afterLoad: function (instance, slide) {
				instance.jumpTo($this.index('[data-fancybox-with-slideshow]'), 0);
			},
