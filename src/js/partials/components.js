class ClickOrSwipeListener {
	...
	onMousedown(eMD) {
		this.timer_id = setTimeout((function (eMD) {
			this.onLongClickTimerEnd(eMD);
		}).bind(this), this.maxDurationOfFastClick);
	...
	onMouseup(eMU) {
		...
		if (Math.abs(this.oldX - eMU.pageX) > 20 || Math.abs(this.oldY - eMU.pageY) > 20) {
	 ...
	 */
	constructor(selector, callOnFastClick, callOnLongClick, callOnSwipe, parentSelector, maxDurationOfFastClick) {
